...
PyObject *CALL_FUNCTION_WITH_ARGS38(PyThreadState *tstate, PyObject *called, PyObject *const *args) {
    CHECK_OBJECT(called);
     CHECK_OBJECTS(args, 38); 
    ...
    if (Nuitka_Function_Check(called)) {
        ...
        if (function->m_args_simple && 38 == function->m_args_positional_count){
            for (Py_ssize_t i = 0; i < 38; i++) {
                Py_INCREF(args[i]);
            }
            result = function->m_c_code(tstate, function,  (PyObject **)args );
        } else if (function->m_args_simple && 38 + function->m_defaults_given == function->m_args_positional_count) {
            NUITKA_DYNAMIC_ARRAY_DECL(python_pars, PyObject *, function->m_args_positional_count);

            memcpy(python_pars, args, 38 * sizeof(PyObject *));
            memcpy(python_pars + 38, &PyTuple_GET_ITEM(function->m_defaults, 0), function->m_defaults_given * sizeof(PyObject *));
            ...
            result = Nuitka_CallFunctionPosArgs(tstate, function, args, 38);
        }
        ...
        if (method->m_object == NULL) {
            ...
            PyObject *result = Nuitka_CallFunctionPosArgs(tstate, method->m_function, args, 38);
            ...
            if (function->m_args_simple && 38 + 1 == function->m_args_positional_count) {
                PyObject *python_pars[38 + 1];
                ...
                result = function->m_c_code(tstate, function, python_pars);
            } else if (function->m_args_simple && 38 + 1 + function->m_defaults_given == function->m_args_positional_count) {
                NUITKA_DYNAMIC_ARRAY_DECL(python_pars, PyObject *, function->m_args_positional_count);

                python_pars[0] = method->m_object;
                Py_INCREF(method->m_object);

                memcpy(python_pars+1, args, 38 * sizeof(PyObject *));
                memcpy(python_pars+1 + 38, &PyTuple_GET_ITEM(function->m_defaults, 0), function->m_defaults_given * sizeof(PyObject *));
                ...
                result = Nuitka_CallMethodFunctionPosArgs(tstate, function, method->m_object, args, 38);
            }
        ...
        if (unlikely(flags & METH_NOARGS)) {
            SET_CURRENT_EXCEPTION_TYPE0_FORMAT1(
                PyExc_TypeError,
                "%s() takes no arguments (38 given)",
                ((PyCFunctionObject *)called)->m_ml->ml_name
            );
            return NULL;
        } else if (unlikely(flags & METH_O)) {
            SET_CURRENT_EXCEPTION_TYPE0_FORMAT1(PyExc_TypeError,
                "%s() takes exactly one argument (38 given)",
                 ((PyCFunctionObject *)called)->m_ml->ml_name
            );
            return NULL;
        } else if (flags & METH_VARARGS) {
...
#if PYTHON_VERSION < 0x360
            PyObject *pos_args = MAKE_TUPLE(args, 38);
            if (flags & METH_KEYWORDS) {
                result = (*(PyCFunctionWithKeywords)method)(self, pos_args, NULL);
            } else {
                result = (*method)(self, pos_args);
...
#if PYTHON_VERSION < 0x370
                result = (*(_PyCFunctionFast)method)(self, (PyObject **)args, 38, NULL);
#else
            PyObject *pos_args = MAKE_TUPLE(args, 38);
                result = (*(_PyCFunctionFast)method)(self, &pos_args, 38);
            Py_DECREF(pos_args);
#endif
            } else {
            PyObject *pos_args = MAKE_TUPLE(args, 38);
...
#else
        PyObject *result = _PyFunction_Vectorcall(called, args, 38, NULL);
#endif
        CHECK_OBJECT_X(result);
            ...
            if (likely(obj != NULL)) {
                ...
                if (NuitkaType_HasFeatureClass(type) && type->tp_init != NULL) {
                    if (type->tp_init == default_tp_init_wrapper) {
                        ...
                        PyObject *result;
                        if (is_compiled_function) {
                            result = Nuitka_CallMethodFunctionPosArgs(tstate, (struct Nuitka_FunctionObject const *)init_method, obj, args, 38);
                        } else {
                            result = CALL_FUNCTION_WITH_ARGS38(tstate, init_method, args);
                            if (init_method_needs_release) {
                                Py_DECREF(init_method);
                            }
                        }
        ...
        if (likely(func != NULL)) {
            PyObject *result = func(called, args, 38, NULL);
