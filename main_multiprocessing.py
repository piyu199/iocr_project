...
logger.basicConfig(
        handlers=[RotatingFileHandler( os.path.join("logs",get_batch_id()+".log"), maxBytes=1000000, backupCount=10)],
    level=logger.INFO,
...
def generate_core_csv_file(input_dir):
    ...
    df.to_csv(f"{batch_id}.csv", index=False, encoding="utf-8")
...
def convert_tif_to_ocr(source_file_path, destination_file_path, lang):
    ...
    if p.returncode != 0:
        logger.info(out.rstrip(), err.rstrip())
...
def process_all_steps(df, df_group, process_number, total_processes, subsets, ocr_lang):
    ...
    for row in df_group.itertuples():
        ...
        if tiffs_count == pdfs_created:
            ...
            combined_pdf_path = os.path.join(destination_directory, output_pdf_name + "_raw_non_pdfa.pdf")
...
def get_no_of_process(df_group): #np.linspace(0,1000,4)
    ...
    no_of_process = 8 if total_unique_folders >= 30 else 6 if total_unique_folders >= 20 else 5 if total_unique_folders >= 10 else 4 if total_unique_folders >= 8 else 3 if total_unique_folders >= 5 else 2 if total_unique_folders >= 2 else 1
    return no_of_process


def get_hardware_information():
    return {"CPU(Physical)": psutil.cpu_count(),
            "CPU(Logical)": psutil.cpu_count(logical=False),
            "RAM(Physical)":f"{psutil.virtual_memory().total / (1024 ** 3):.2f} GB"}


def process_ocr(ocr_lang):
    ...
    if first_filename and final_filename:
        ...
        no_of_workers = get_no_of_process(df_group.copy())
        subsets = np.linspace(0, df_group.shape[0], no_of_workers+1)
        subsets = [int(x) for x in subsets]
        pool = Pool(no_of_workers)
        workers = [pool.apply_async(process_all_steps,
                                    args=(df.copy(), df_group.copy(), i, no_of_workers, subsets, ocr_lang)) for i in range(1, no_of_workers+1)]
        for w in workers:
...
def convert_pdf_to_pdfa(source_file, destination_file):
    ...
    try:
        ...
        logger.info(f"Error:source_file : {source_file}, destination_file: {destination_file}")
...
if __name__ == '__main__':
    ...
    layout = [
        ...
        [psg.Text(text='Please provide input to run OCR model', size=20, expand_x=True, justification='left', text_color="Green")],
            ...
            if os.path.exists(INPUT_DIR) and os.path.exists(OUTPUT_DIR):
                if len(OCR_LANG) != 0:
                    ...
                    if is_okay_to_proceed:
                        ...
                        window['processing'].update(f"Finished OCR  of {source_dir_size :.2f} GB, Total Time Taken {end_time - start_time}")
